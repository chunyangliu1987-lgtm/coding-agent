...
async def search_branches(
    ...
    if query:
        page, gh_after = decode_branch_search_continuation(page_id)
        outcome = await client.search_branches_page(
            selected_provider=provider,
        ...
        next_page_id = None
        if provider == ProviderType.GITHUB and outcome.github_next_after_cursor:
            next_page_id = encode_github_branch_search_continuation(
                outcome.github_next_after_cursor
            )
        elif len(outcome.branches) > limit:
            next_page_id = encode_page_id(page + 1)
